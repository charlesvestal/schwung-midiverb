...
static inline uint16_t lfo_triangle_update(Lfo *lfo) {
    if (lfo->dir) {
        lfo->value += lfo->rate;
...
static inline void lfo_triangle_reset(Lfo *lfo) {
    lfo->dir = 1;
    lfo->value = 0;
}

static inline uint16_t lfo_sine_update(Lfo *lfo) {
...
static inline void lfo_sine_reset(Lfo *lfo) {
    lfo->x = 0;
